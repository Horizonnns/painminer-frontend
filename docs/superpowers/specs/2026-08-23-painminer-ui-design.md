...
Вход идёт шагами: ключи приложения (если их ещё нет) → телефон → код →
пароль двухфакторной защиты, если он включён.
...
* `api_id`/`api_hash` пишутся в `.env` с правами 600 — это не вход в аккаунт,
  а лишь право клиента представиться Telegram; получить их программно нельзя;
...
| POST | `/auth/credentials` | сохранить api_id/api_hash в `.env` |
