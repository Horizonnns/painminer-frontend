...
**Не входит в задачу:** вход в Telegram через веб, многопользовательский режим,
профили авторов сообщений, отправка чего-либо в Telegram.
...
Логин (телефон, код, 2FA-пароль) остаётся в терминале: `painminer login` или
первый `scan`. Через HTTP коды подтверждения не передаются — удобство не стоит
риска. Сервер лишь проверяет наличие session-файла и возвращает `no_session`,
если его нет; интерфейс показывает, какую команду выполнить.
...
- Логин в Telegram только через терминал.
